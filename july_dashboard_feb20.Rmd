...
grantee %>%
  mutate(Gender = replace(Gender, Gender=="M", "Male"),
         ...
         Nationality = replace(Nationality,Nationality == "Mauritian", "Mauritius"),
          Nationality = replace(Nationality,Nationality == "Zimbabwe", "South Africa")) %>%
  separate(col = Nationality, into = c("Nationality", "b"), sep = " ") %>%
...
marg1.link = marg1 %>%
  ...
  select(Year, Grant = `Grant Type`, Title, Name, Grantee = Contact, Gender, Country = Nationality, Institution, `Research Focus` = `Research Title`)

marg2.link =    marg2 %>%
  separate(col = `Full Name`, into = c("Title", "Name"), sep = 3) %>%
  mutate(Contact = paste0('<a  target=_blank href=', Contact, '>', Name,'</a>' )) %>% 
  select(Year, Grant = `Grant Type`, Name, Title, Grantee = Contact, Gender, Country = Nationality, Institution = `Home Institution`, `Research Focus` = `Research Title`)

marg3.link =  marg3 %>%
  separate(col = `Full Name`, into = c("Title", "Name"), sep = 3) %>%
  mutate(Contact = paste0('<a  target=_blank href=', Contact, '>', Name,'</a>' )) %>% 
  select(Year, Grant = `Grant Type`, Name, Title, Grantee = Contact, Gender, Country = Nationality, Institution = `Home Institution`, `Research Focus` = focus)

marg1.2020.link =margs2020 %>%
  # separate(col = Name, into = c("Title", "Name"), sep = 3) %>%
  mutate(Title = "", email = "",Contact = paste0('<a  target=_blank href=', email, '>', Name,'</a>')) %>% 
  select(Year, Grant = `Grant Type`, Name, Title, Grantee = Contact, Gender, Country = Nationality, Institution, `Research Focus` = focus)
...
grantee.country.stats = grantee %>%
  ...
  group_by(country, grant_type) %>% 
  count() %>% ungroup()%>%
  separate(col = country, into = c("country", "b"), sep = -6) %>%
  select(-b) 

grantee.country.stats.wide = grantee.country.stats%>%
  pivot_wider(names_from = grant_type, values_from = n) %>%
  janitor::clean_names()  %>%
  rowwise %>%
  mutate(total = sum(marg_i, marg_ii, marg_iii, na.rm = TRUE))

africa = st_read("e:/GIS/Tanzania spatial data Bank/EAF14 Tanzania spatial datasets/africa/Spatial/AdmInfr/afcntry.shp", quiet = TRUE) %>% select(country = CNTRY_NAME)

grantee.country.stats.wide = tibble(country = c(grantee.country.stats.wide$country),
       lon = c(39.7,43.7, 57.5, 32.6,55.45655, 26.51625, 39.19186),
       lat = c(-4.06,-23.4, -20.2,-26.0, -4.630438, -33.31355, -6.849969)) %>%
  bind_cols(grantee.country.stats.wide %>% select(-country))
...
basemap %>%
  addMinicharts(lng = grantee.country.stats.wide$lon, 
                lat = grantee.country.stats.wide$lat, 
                # type = "bar", 
                chartdata = grantee.country.stats.wide$total, 
                showLabels = TRUE, 
                width = 55)
...
basemap %>%
  addMinicharts(lng = grantee.country.stats.wide$lon, 
                lat = grantee.country.stats.wide$lat, 
                type = "pie", 
                chartdata = grantee.country.stats.wide[, c("marg_i", "marg_ii", "marg_iii")],
                showLabels = TRUE,
                colorPalette = wiomsa.colors$colors[c(1,3,6)], 
                width = 60 * sqrt(grantee.country.stats.wide$total) / sqrt(max(grantee.country.stats.wide$total)), 
                transitionTime = 0)
```


```{r, eval=FALSE}
grantee.type.year = grantee %>% janitor::clean_names() %>%
  group_by(grant_type, year, nationality) %>%
  count() %>% 
  rename(country = nationality) %>%
  separate(col = country, into = c("country", "b"), sep = -6) %>%
  select(-b)  %>% pivot_wider(names_from = grant_type, values_from = n) %>%
  left_join(grantee.country.stats.wide %>% select(1:3)) %>%
  janitor::clean_names()


basemap %>%
  addMinicharts(lng = grantee.type.year$lon, 
                lat = grantee.type.year$lat, 
                type = "bar", 
                chartdata = grantee.type.year[, c("marg_i", "marg_ii", "marg_iii")],
                time = grantee.type.year$year,
                # showLabels = TRUE,
                colorPalette = wiomsa.colors$colors[c(1,3,6)], 
                width = 45, height = 65)
...
This dashboard provides interactive plots and maps that highlight Marine Research Grants (MARGS) work in the Western Indian Ocean Region.You can navigate the tool using standard menu, which serves as pages. You can zoom and pan to the area of interest within the region using the tools of an interactive map. The dashboard is under active development by WIOMSA. We also express our profound gratitude to  individuals who helped to develop and put the tool up and running. Several open source tools were used during the development stage.
